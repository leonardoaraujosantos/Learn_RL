...
def tf_policy_forward(state_input):  # x ~ [1,D]
    h = tf.matmul(state_input, tf_model['W1'])
    h = tf.nn.relu(h)
...
# Prepare game frame (Crop, downsample)
def prepro(image_in):
    """ prepro 210x160x3 uint8 frame into 6400 (80x80) 1D float vector """
    image_in = image_in[35:195]  # crop
    image_in = image_in[::2, ::2, 0]  # downsample by factor of 2
    image_in[image_in == 144] = 0  # erase background (background type 1)
    image_in[image_in == 109] = 0  # erase background (background type 2)
    image_in[image_in != 0] = 1  # everything else (paddles, ball) just set to 1
    return image_in.astype(np.float).ravel()
