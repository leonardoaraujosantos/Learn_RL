...
class agent():
    def __init__(self, lr, s_size, a_size, h_size):
        ...
        # Network FC-->Relu-->FC-->Softmax
        hidden = slim.fully_connected(self.state_in, h_size, biases_initializer=None, activation_fn=tf.nn.relu)
        self.output = slim.fully_connected(hidden, a_size, activation_fn=tf.nn.softmax, biases_initializer=None)

        # Choose action with biggest probability
...
with tf.Session() as sess:
    ...
    while i < total_episodes:
        ...
        for j in range(max_ep):
            ...
            a_dist = sess.run(myAgent.output, feed_dict={myAgent.state_in: [curr_state]})
            ...
            # Save some experience
            ep_history.append([curr_state, action, reward, next_state])
